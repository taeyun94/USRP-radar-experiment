...
load rx_field_speed_v3.mat
iter=size(total_rx_sig);
rx_sig_delay=total_rx_sig;
Fc=5.8; % in Hz

for loop=1:iter(3)
    [output,peak]=cross_corr(tx_sym,rx_sig_delay(:,2,loop));
    
    ch_out=rx_sig_delay(peak-1098+1:peak-1098+length(tx_sym),1,loop);
    ...
    view(2);
    axis([-200 200 0 500 min(min(data_set)) max(max(data_set))])
    figure(2);hold off;
    imshow(data_set);
    view(2);
end
